...
//rows.each { row ->
	...
	try {
		...
		SS.screenshotDir(row['TC_ID'])   // Defined screenshot directory
		
		login_Page.login(row['TC_ID'])
		
		Home_Page.chooseProposalType(row['TC_ID'])
		
		LA_Details_Page.fillLADetails(row['TC_ID'])
		Map homeData = FetchExcelData.getData("Home_Page", row['TC_ID'])  // Fetching data from home page sheet to execute proposer details if life type is other life
		
		if(homeData['Life_Type'] == 'Other Life') {
			
			Proposer_Details_Page.fillProposerDetails(homeData['Proposal_Type'], row['TC_ID'])
			
		}
		
		Address_Details_Page.fillAddressDetails(row['TC_ID'])
		...
		SourceBy_Page.fillSourceDetails(row['TC_ID'])
		
		Medical_Details_Page.fillMedicalDetails(row['TC_ID'])
		
		Other_Details_Page.fillFamilyDetails(row['TC_ID'])
		
		if(homeData['Life_Type'] == 'Own Life') {
			
			Nominee_Details_Page.fillNomineeDetails(row['TC_ID'])
			
		}
		
		Declaration_Page.fillDetails(homeData['Life_Type'], row['TC_ID'], homeData['Proposal_Type'])
		
		Documents_Page.uploadAllDocuments(homeData['Life_Type'], row['TC_ID'])
		
		Map planData = FetchExcelData.getData("Plan_Details_Page", row['TC_ID'])
		
		if(planData['Frequency'] != 'Single') {
			NACH_Registration_Page.fillNachDetails(row['TC_ID'])
		}
		
		Summary_Page.checkSummaryPage()
		
		Payment_Page.processPayment(row['TC_ID'])
