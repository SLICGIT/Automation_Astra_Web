...
//		JOptionPane.showMessageDialog(null, 'Click OK to continue')
		...
		WebUI.callTestCase(findTestCase('Test Cases/Old Test Cases/Plan Details Page/SIPV0X'),
			[
...
//                valueToSelect) + '\']')))
    ...
    for (int i = 0; i < 50; i++) {
        ...
        try {
			...
			def optionXPath = '//div[contains(@class,\'ant-select-item-option-content\') and text()=\'' + valueToSelect + '\']'
		...
		try {
			...
			def optionXPath = '(//div[contains(@class,\'ant-select-item-option-content\') and text()=\'' + valueToSelect + '\'])[2]'
...
def getBI() {
	...
	// Perform UI actions
	WebUI.enhancedClick(findTestObject('PlanDetails/BIDownloadYes', [('BI_Required') : BI_Required]))
