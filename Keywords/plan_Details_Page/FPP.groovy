...
public class FPP {
	...
	static def fillPlanDetails(String testcaseID) {
		...
		if(GlobalVariable.planName.toString().contains('V02')) {
			...
			dropdowns.selectDropdownValue('Maturity', data['Maturity_Benefit'])

			if(data['Maturity_Benefit'] == 'Installment') {
				dropdowns.selectDropdownValue('Installment Mode', data['Maturity_Payout'])
			}
			...
			if(data['Death_Benefit'].toString().contains('Installment')) {
