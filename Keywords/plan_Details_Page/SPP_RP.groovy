...
public class SPP_RP {
	...
	static def fillPlanDetails(String testcaseID) {
		...
		if(GlobalVariable.planName.toString().contains('V0X')) {
			...
			if(data['Death_Benefit'].toString().contains('Installments')) {
			...
			dropdowns.selectDropdownValue('Life Cover Type', data["Life_Cover"])
			...
			if(data["Death_Benefit"].toString().contains("Installments")) {
				dropdowns.selectDropdownValue('Death Payout Option', data["Death_Payout"])
				dropdowns.selectDropdownValue('Death Installment', data["Death_Payout_Period"])
			}
