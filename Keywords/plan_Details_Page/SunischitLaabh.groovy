...
public class SunischitLaabh {
	...
	static def fillPlanDetails(String testcaseID) {
		...
		dropdowns.selectAntDropdown('MaturityPayoutMode', data['Maturity_Payout'])

		dropdowns.selectAntDropdown('MatPayoutPeriod', data['Maturity_Payout_Period'])

		dropdowns.selectAntDropdown('Deathbenefit', data['Death_Benefit'])

		if(data['Death_Benefit'] == 'Life Plus Option (with in-built Waiver of Premium)-Installments' && !data["Plan_Name"].toString().contains('POS')) {
			dropdowns.selectAntDropdown('DeathPayoutMode', data['Death_Benefit'])
		}

		dropdowns.selectAntDropdown('SAMultiplier', data['SA_Multiplier'])

		dropdowns.selectDropdownValue('Premium Frequency', data['Frequency'])
		...
		if(!data["Plan_Name"].toString().contains('POS')) {
